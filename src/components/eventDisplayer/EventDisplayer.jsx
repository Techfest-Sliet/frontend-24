...
const EventDisplayer = () => {
        ...
        const getProperTeam = async () => {
                fetch(`${baseUrl}/team`, { credentials: "include" }).then(v => v.json()).then((v) => {
                        setTeamDetails(v)
                }).catch(console.error)
        };
        ...
        async function registerEvent(participationType) {
                ...
                setIsLoading(true);
                fetch(`${baseUrl}/event/join/individual`, { method: "POST", credentials: "include", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ id: eventId }) })
        }
