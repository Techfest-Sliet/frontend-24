...
const EventDisplayer = () => {
  ...
  useEffect(() => {
    const getTeam = async () => {
        await fetch(`${baseUrl}/profile`, { credentials: "include" })
      .then((r) => {
        r.ok &&
  ...
  if (teams && teams[0] && !teams[0].members) {
    Promise.all(
      teams.map((t) =>
        fetch(`${baseUrl}/team/member?id=${t.id}`, { credentials: "include" })
          .then((v) => v.json())
          .then((v) => {
            t.members = v;
            return t;
  ...
  /*
    const getProperTeam = async () => {
        ...
        //     })
        //     .catch(console.error);
        const team = await fetch(`${baseUrl}/team`, { credentials: "include" })
        if(!team.ok){
            console.log(team.statusText)
            return;
        }
                                  ...
                                  >
									...
									{isTeams &&
                                      ...
                                      <Typography style={{ fontSize: "60%" }}>
                                        Note: Every Team Member should verify
                                        the team through team invitation,
                                        otherwise team list will not be shown.
                                      </Typography>
                                      <Button
                                        variant="contained"
                                        style={{
